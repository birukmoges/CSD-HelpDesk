...
import { AiOutlineClose, AiOutlineUser, AiOutlineLock ,AiOutlineEye,AiOutlineEyeInvisible} from 'react-icons/ai';
...
const Login = ({ theme }) => {
  ...
  const handleVerifyOTP = async (enteredOTP) => {
    try {
      ...
      if (verifyOTPResponse.data.message.includes('successfully')) {
    ...
    if (isLogin) {
      ...
      try {
        ...
        if (response.data.success) {
          // Registration succeeded, show login popup
...
<div className={`relative flex items-center bg-${theme.colors.background} rounded-full mb-4 p-2`}>
  <AiOutlineLock size={25} />
  
  {/* Input field with extra padding to avoid text overlapping with the button */}
  <input
    className={`bg-transparent p-2 w-full focus:outline-none ml-2 text-${theme.colors.text} pr-10`}
  ...
  {/* Eye icon button positioned inside the input field */}
  <button
    type="button"
    className="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-500 border-none outline-none focus:ring-0"
